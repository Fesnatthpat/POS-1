        ...
        <table class="w-full text-left border-collapse min-w-[700px] lg:min-w-0">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              <th class="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px]">Member</th>
              <th class="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px]">Contact</th>
              <th class="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px]">Level</th>
              <th class="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-[10px]">Points</th>
              <th class="px-6 py-4 font-bold text-slate-500 uppercase tracking-wider text-right text-[10px]">Actions</th>
            </tr>
          </thead>
            ...
            <tr v-for="customer in customers" :key="customer.id" class="hover:bg-slate-50/80 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center space-x-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-[9px] lg:text-[10px] text-slate-400 font-bold uppercase tracking-widest truncate">Joined: {{ customer.joinDate }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <div class="flex items-center space-x-1 text-indigo-600 font-black text-xs lg:text-sm">
                  ...
                  <span class="text-[9px] lg:text-[10px] uppercase">pts</span>
                </div>
              </td>
    ...
    <div v-if="isAddModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div class="p-6 lg:p-8">
          <h3 class="text-xl lg:text-2xl font-black text-slate-900 tracking-tight mb-6">New Membership</h3>
          <form @submit.prevent="handleAddCustomer" class="space-y-5">
            ...
            <div>
              <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Phone</label>
              <input type="tel" required v-model="newCustomer.phone"
                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                placeholder="0xx-xxx-xxxx" />
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Level</label>
